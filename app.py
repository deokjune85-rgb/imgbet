...
<style>
    ...
    /* 6. Legal Shield Box (스크롤 가능한 약관 박스) */
    .legal-box {
...
def legal_disclaimer_gate():
    ...
    st.markdown("<h3 style='text-align: center; color: #888;'>서비스 이용 약관 및 면책 고지</h3>", unsafe_allow_html=True)
    
    st.warning("⚠️ 본 서비스는 만 19세 이상만 이용 가능합니다.")

    # [★수정★] 스크롤 가능한 약관 박스 (법적 보호용 긴 텍스트)
    st.markdown("""
    <div class="legal-box">
        <strong>제1조 (목적)</strong><br>
        본 약관은 Veritas Sports AI(이하 "회사")가 제공하는 스포츠 데이터 분석 정보(이하 "서비스")의 이용 조건을 규정합니다.<br><br>
        <strong>제2조 (서비스의 성격)</strong><br>
        1. 본 서비스는 AI 알고리즘을 통해 스포츠 경기 데이터를 통계적으로 분석한 단순 '참고 정보'만을 제공합니다.<br>
        2. 회사는 국민체육진흥법을 준수하며, 불법 사설 스포츠 도박 사이트의 이용을 엄격히 금지합니다.<br>
        3. 본 서비스가 제공하는 정보는 승패를 보장하지 않으며, 이를 활용한 베팅의 결과에 대한 책임은 전적으로 이용자 본인에게 있습니다.<br><br>
        <strong>제3조 (책임의 면제)</strong><br>
        회사는 본 서비스의 정보 오류, 누락, 지연 및 이를 이용한 투자/베팅 결과로 인한 어떠한 금전적, 정신적 손해에 대해서도 법적 책임을 지지 않습니다.<br><br>
        <strong>제4조 (환불 규정)</strong><br>
        본 서비스는 '디지털 콘텐츠'로서, 유료 정보(VIP 코드) 열람 시 데이터 가치가 즉시 소멸되므로 전자상거래법에 의거하여 환불이 불가능합니다.<br><br>
        <strong>제5조 (저작권)</strong><br>
        제공된 분석 데이터의 무단 복제, 배포, 공유를 금지하며 적발 시 민형사상 책임을 물을 수 있습니다.
    </div>
    """, unsafe_allow_html=True)

    with st.form(key='agreement_form'):
        agree1 = st.checkbox("[필수] 위 이용 약관 및 면책 조항을 모두 읽었으며 이에 동의합니다.")
        agree2 = st.checkbox("[필수] 본인은 만 19세 이상이며, 불법 도박 목적으로 이용하지 않겠습니다.")
        agree3 = st.checkbox("[필수] 정보의 결과에 대해 회사에 민형사상 책임을 묻지 않겠습니다.")

        submit_button = st.form_submit_button(label='동의하고 입장하기 (Enter System)')
        ...
        if submit_button:
            if agree1 and agree2 and agree3:
                ...
                st.error("모든 필수 항목에 동의하셔야 입장이 가능합니다.")
    st.stop()
    ...
    for i, msg in enumerate(st.session_state.chat_history):
        with st.chat_message(msg["role"], avatar="✨" if msg["role"]=="assistant" else "👤"):
            if msg["role"]=="assistant" and not msg.get("animated") and i==len(st.session_state.chat_history)-1:
                ...
                st.markdown(f"<div class='ai-gradient-text'>{msg['content']}</div>" if msg["role"]=="assistant" else msg["content"], unsafe_allow_html=True)
